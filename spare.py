...
for fname in iglob(os.path.expanduser('output1/*.json')):
    with open(fname) as fin:
        ...
        n1= graph.find_one("Ariana",property_key = 'name', property_value = auth_name)
        #if(dt>n1['date']):
         #   n1['date']=dt
        nh=[]
        ...
        if (mn_dict !=None):
            ...
            for ment in mn_dict:
                ...
                print (m_name)
                if(graph.find_one("Ariana",property_key = 'name', property_value =m_name)==None):
                    node = Node("Ariana", name=m_name,type=3)
                    graph.create(node)
                n3= graph.find_one("Ariana",property_key = 'name', property_value =m_name )
                nm.append(n3)
